...
public class OfferResourceIT {
    ...
    public void setup() {
        ...
        final OfferResource offerResource = new OfferResource(offerService, offerQueryService);
    ...
    @Test
    @Transactional
    public void getAllOffersByOptionsIsEqualToSomething() throws Exception {
        // Initialize the database
        offerRepository.saveAndFlush(offer);
        Option options = OptionResourceIT.createEntity(em);
        em.persist(options);
        em.flush();
        offer.addOptions(options);
        offerRepository.saveAndFlush(offer);
        Long optionsId = options.getId();

        // Get all the offerList where options equals to optionsId
        defaultOfferShouldBeFound("optionsId.equals=" + optionsId);

        // Get all the offerList where options equals to optionsId + 1
        defaultOfferShouldNotBeFound("optionsId.equals=" + (optionsId + 1));
    }
